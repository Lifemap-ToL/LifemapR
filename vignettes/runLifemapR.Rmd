...
vignette: >
  ...
  %\VignetteIndexEntry{1-Use of LifemapR} 
...
The very first step to use this package is to have a dataframe containing at least a ```taxid``` column containing NCBI format TaxIds. This dataframe can also contain other datas that you might want to visualise.
...
- ```basemap``` : which basemap was used to fetch the datas.

You can then visualise your datas with ```lifemap()``` associated with one or more of the following functions :

- ```lm_markers()```, to represent datas as circles.
- ```lm_branches()```, to represent the data's subtree.
- ```lm_discret()```, to represent discret datas as piecharts.
...
These function also allow the user to represent datas by modifying characteristics of representations as we'll see later in the examples.
...
Then we can began to visualize our datas.
...
```{r, echo=FALSE,out.width="40%", out.height="20%",fig.cap= 'visualisation of kraken datas',fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE,out.width="40%", out.height="20%",fig.cap= 'left : display = "requested", right : display = "leaves"',fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE,out.width="40%", out.height="20%",fig.cap= 'Usage of the ```popup``` argument',fig.show='hold',fig.align='center'}
knitr::include_graphics("figures/kraken_popup.png")
``` 

Finally, it is also possible to represent datas with a subtree, either by the size of the branches or their color.
...
```{r, echo=FALSE,out.width="45%", out.height="30%",fig.cap= "left : branche's color, right : branche's size",fig.show='hold',fig.align='center'}
...
However, unlike the precedent data set, we don't have informations for all the nodes. Here we only have datas for the leaves so it will be necessary to infere values to the nodes where the information is missing with the ```FUN``` argument.
...
```{r, echo=FALSE,out.width="100%", out.height="20%",fig.cap= 'visualisation of genomics datas',fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE,out.width="100%", out.height="20%",fig.cap= 'visualisation of genomics datas with markers and subtree',fig.show='hold',fig.align='center'}
...
```{r, eval = FALSE}
# Visualisation of eukaryotes datas.
lifemap(LM_eukaryotes) + 
  lm_markers()

```

```{r, echo=FALSE,out.width="50%", out.height="20%",fig.cap= 'Basic visualisation',fig.show='hold',fig.align='center'}
knitr::include_graphics("figures/eukaryotes_base.png")
``` 


We can also choose to visualise only a part of our datas. To do this, we can either sort our datas in advance or use the ```data``` argument to do so.
...
```{r, echo=FALSE,out.width="50%", out.height="20%",fig.cap= 'Visualisation of Plants',fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE,out.width="50%", out.height="20%",fig.cap= 'Visualisation of the maximum assembly level',fig.show='hold',fig.align='center'}
